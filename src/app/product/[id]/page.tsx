...
export default function ProductPage({ params }: PageProps) {
  return (
    <main className="mx-auto w-full max-w-[720px] flex-1 pb-32">
...
function ProductSkeleton() {
  return (
    <div aria-hidden>
      <div className="aspect-[4/3] w-full bg-white sm:mt-6 sm:rounded-card" />
...
async function ProductBody({ params }: PageProps) {
  ...
  return (
    <>
      <div className="relative aspect-[4/3] w-full overflow-hidden sm:mt-6 sm:rounded-card">
        <ProductPhoto
          name={product.name}
      ...
      <div className="px-5">
        ...
        <div className="mt-6 flex items-center justify-between rounded-card bg-white px-5 py-4 shadow-soft">
        ...
        {facts.length > 0 && (
          <dl className="mt-3 divide-y divide-line rounded-card bg-white px-5 shadow-soft">
            {facts.map(([label, value]) => (
              <div key={label} className="grid grid-cols-[92px_1fr] gap-3 py-4">
        ...
        <p className="pt-6 text-center text-[12.5px] leading-relaxed text-ink-faint">
